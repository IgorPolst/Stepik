...
with open(
    "Stepik\Level_profi\Work_with_file\JSON\step_14\exam_results.csv",
    "rt",
    encoding="utf-8",
) as file:
...
for student in data:
    ...
    best = sorted(
        filtered, key=lambda el: (el["score"], el["date_and_time"]), reverse=True
    )[0]
...
with open(
    "Stepik/Level_profi/Work_with_file/JSON/step_14/best_scores.json",
    "wt",
    encoding="utf-8",
) as file:
